...
# Dictionary to track configured meters
configured_meters = {}
...
while True:
    ...
    msg = {
        ...
        "Consumption": 0,
        "Unit": "Unknown",
        "LeakNow": "None" # Initialize LeakNow
    }
    ...
    # SCM+ messages
        ...
        if data['Message']['EndpointType'] in (4, 5, 7, 8, 110):  # electric meter
            ...
            msg['Consumption'] = data['Message']['Consumption'] / METERMON_WATER_DIVISOR  # convert to gal
            msg['Unit'] = "gal"
            msg['LeakNow'] = "None" if data['Message'].get('Leak') is None else "Leak"  # Check for leak
    ...
    if msg['Consumption'] >= 0:  # Allow for zero consumption
        meter_id = msg['ID']
        ...
        if meter_key not in configured_meters:
            ...
            consumption_config_payload = json.dumps({
                ...
                "availability_topic": f"{MQTT_TOPIC_PREFIX}/status", # Use the defined prefix
            ...
            #Publish state to config (This needs to happen *after* the config message)
        ...
        leak_state_payload = json.dumps({
            ...
            "leak_now": msg['LeakNow']  # Now correctly populated
        })
